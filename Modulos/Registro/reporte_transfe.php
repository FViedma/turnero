...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->SetTextColor(90, 90, 90);
        $this->Cell(0, 10, utf8_decode("FORMULARIO TRANSFERENCIA"), 0, 0, 'C');
        $this->Ln(6);
        ...
        $this->SetTextColor(90, 90, 90);
        include "conexion.php";
        $consulta = "SELECT * FROM transferen1 ";
        if (isset($_POST['reimpresion'])) {
            $ci = $_POST['ci'];
            $fecha = $_POST['fecha_formulario'];
            $consulta = sprintf("SELECT * FROM transferen WHERE ci=%d AND fecha='%s'", $ci, $fecha);
        }
        ...
        while ($datos = mysqli_fetch_assoc($res)) {
            if (isset($datos['id_transferen1'])) {
                $nro_transferencia = $datos['id_transferen1'];
            } else {
                $nro_transferencia = $datos['id_transferen'];
            }
            $this->Cell(0, 10, utf8_decode("N° TRANSFERENCIA:" . $nro_transferencia), 0, 0, 'R');
        }
        $Object = new DateTime();
        $fech = $Object->format("d-m-Y h:i:s a");
        $this->ln();
        $this->Cell(0, 5, utf8_decode("FORMULARIO D7b TRANSFERENCIA"), 0, 0, 'C');
...
while ($datos = mysqli_fetch_assoc($res)) {
    ...
    $pdf->Ln(1);
    $pdf->Cell(0, 3, utf8_decode("DATOS DEL ESTABLECIMIENTO DE SALUD QUE TRANSFIERE(C1)"), 0, 0, 'C', true);
    $pdf->Ln(3.5);
    $pdf->SetFont('Arial', 'B', 6);
    $pdf->SetTextColor(90, 90, 90);
    $pdf->Cell(55, 5, utf8_decode("NOMBRE DEL ESTABLECIMIENTO QUE TRANSFIERE:"), 0, 0, 'C');
    $pdf->Cell(55, 5, utf8_decode($datos['establecimiento_salud']), 0, 0, 'C');
    ...
    $pdf->Cell(45, 15, utf8_decode('SELLO DEL EESS QUE TRANSFIERE -->'), 1, 0, 'R');


    $pdf->Ln(2);
    $pdf->Cell(16, 5, utf8_decode("NIVEL DE EESS:"), 0, 0, 'C');
    ...
    $pdf->Ln(2);
    $pdf->Cell(15.5, 5, utf8_decode("TOMOGRAFIA"), 0, 0, 'C');
    $pdf->Cell(50, 5, utf8_decode($datos['tomografia']), 0, 0, 'C');
    $pdf->Ln(2);
    ...
    $pdf->Ln(2);
    $pdf->Cell(5, 5, utf8_decode("d)"), 0, 0, 'C');
    $pdf->Cell(135, 5, utf8_decode($datos['diagnostico_pres4']), 0, 0, 'L');
    $pdf->Cell(1, 5, utf8_decode("CIE - 10"), 0, 0, 'R');
    ...
    $pdf->SetFont('Arial', 'B', 6);
    $pdf->SetTextColor(90, 90, 90);
    $pdf->Cell(170, 5, utf8_decode($datos['tratamiento1']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['tratamiento2']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['tratamiento3']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['tratamiento4']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['tratamiento5']), 0, 0, 'C');

    $pdf->Ln(12);
    $pdf->Cell(17, 5, utf8_decode("OTROS ANEXOS"), 0, 0, 'C');
    $pdf->Cell(21, 5, utf8_decode($datos['otros_anexos']), 0, 0, 'C');
    $pdf->Cell(1, 5, utf8_decode("CUALES"), 0, 0, 'C');
    $pdf->Cell(40, 5, utf8_decode($datos['cuales']), 0, 0, 'C');
    $pdf->Ln(4);
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->SetTextColor(255, 255, 255);
    ...
    $pdf->SetFont('Arial', 'B', 6);
    $pdf->SetTextColor(90, 90, 90);
    $pdf->Cell(170, 5, utf8_decode($datos['observaciones1']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['observaciones2']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['observaciones3']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['observaciones4']), 0, 0, 'C');
    $pdf->Cell(170, 5, utf8_decode($datos['observaciones5']), 0, 0, 'C');
    $pdf->Ln();
    $pdf->Cell(35, 5, utf8_decode("MOTIVO DE TRANSFERENCIA:"), 0, 0, 'C');
    ...
    $pdf->Ln(2);
    $pdf->Cell(104, 5, utf8_decode("habiendome informado sobre el cuadro clinico, autorizo al medico tratante y personal de salud del establecimiento,realizar la transferencia, teniendo en cuenta que he sido "), 0, 0, 'L');
    ...
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->SetTextColor(255, 255, 255);
    $pdf->Cell(0, 3, utf8_decode("MOTIVO DE TRANSFERENCIA(C11)SOLO MARQUE UNO(10):"), 0, 0, 'C', TRUE);
    $pdf->Ln(2);
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->SetTextColor(90, 90, 90);
    ...
    $pdf->Cell(15, 5, utf8_decode($datos['serv_espe']), 0, 0, 'C');
    ...
    $pdf->Cell(35, 5, utf8_decode($datos['nombre_contact']), 0, 0, 'C');

    $pdf->Cell(18, 5, utf8_decode("MEDIO DE COMUNICACION:"), 0, 0, 'C');
    $pdf->Cell(35, 5, utf8_decode($datos['medio_comunicacion']), 0, 0, 'C');

    $pdf->Cell(5, 5, utf8_decode("REPORTADO AL CCESA-DA:"), 0, 0, 'C');
    $pdf->Cell(35, 5, utf8_decode($datos['reportado_cces']), 0, 0, 'C');
    $pdf->Ln(2);
    $pdf->SetFont('Arial', 'B', 6);
    $pdf->Cell(45, 5, utf8_decode("NOMBRE DE QUIEN RECIBE AL PACIENTE:"), 0, 0, 'C');
    $pdf->Cell(35, 5, utf8_decode($datos['nombre_resc_paci']), 0, 0, 'C');

    $pdf->Cell(5, 5, utf8_decode("FECHA DE RECEPCION:"), 0, 0, 'C');
    $pdf->Cell(48, 5, utf8_decode($datos['fecha_recep']), 0, 0, 'C');

    $pdf->Cell(5, 5, utf8_decode("HORA DE RECEPCION:"), 0, 0, 'C');
    $pdf->Cell(35, 5, utf8_decode($datos['hora_recep']), 0, 0, 'C');
    $pdf->Ln(2);
    ...
    $pdf->Cell(35, 5, utf8_decode($datos['paciente_admit']), 0, 0, 'C');

    $pdf->Cell(5, 5, utf8_decode("MOTIVO:"), 0, 0, 'C');
    $pdf->Cell(35, 5, utf8_decode($datos['motivo']), 0, 0, 'C');
    $pdf->Ln(7);
    $pdf->Cell(5, 5, utf8_decode(""), 0, 0, 'C');
